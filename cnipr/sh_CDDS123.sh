...
ID=`ps -ef | grep "cmd_CDDS123.py" |grep -v "grep"| awk '{print $2}'`
echo "first time $ID"
if [ "$ID" ]
then
    ...
    while true
    do
        IDD=`ps -ef | grep "cmd_CDDS123.py" |grep -v "grep"| awk '{print $2}'`
...
nohup python cmd_CDDS123.py >> out_CDDS123.out 2>&1 &