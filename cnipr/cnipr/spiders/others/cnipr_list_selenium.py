...
class TouzishijianSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for tr in tr_tags:
			...
			aux = '; '.join([self._solSpace(au) for au in auxs if au]) if auxs else ''
			atexts = tr.xpath('./td/a//text()').extract()
			atext = ''.join([self._solSpace(at) for at in atexts if at]) if atexts else ''
			tdtexts = tr.xpath('./td[2]/text()').extract()
			tdtext = ''.join([self._solSpace(td) for td in tdtexts if td]) if tdtexts else ''
			if '申请日' in text:
				applicatDate = aux
			elif '主分类号' in text:
				mainClassNum = aux
		...
		desc_pics = select.xpath('//*[@id="ftul"]//a/@href').extract()
		desc_pics = '; '.join(desc_pics)
		item = CniprItem()
		...
		item['apply_pdf_url'] = apply_pdf_url
		item['author_pdf_url'] = author_pdf_url
		item['abs_pic'] = abs_pic
		item['desc_pics'] = desc_pics
	...
	def legal(self, response):
		...
		listLegalInfo = text.get('listLegalInfo')  # 列表，是不是可以直接json dumps？
		legal_dict = {'listLegalInfo': listLegalInfo}
	...
	def cnReference(self, response):
		...
		cn_dict = {'sqryzzlList': sqryzzlList}
	...
	def patentList(self, response):
		...
		patentList_dict = {'patentList': patentList}
