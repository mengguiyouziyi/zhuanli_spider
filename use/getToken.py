...
def get_token(path='/home/spider/.pyenv/versions/3.5.3/bin/phantomjs'):
	...
	# browser = webdriver.Chrome(executable_path='/root/.pyenv/versions/3.5.4/bin/chromedriver')
	...
	# print(m.text)
	browser.find_element_by_id('resource_获取Authorization_Code').find_element_by_class_name('collapseResource').click()
	time.sleep(0.5)
	browser.execute_script("window.scrollTo(document.body.scrollWidth, document.body.scrollHeight)")
	time.sleep(0.5)
	browser.find_element_by_id('resource_获取Authorization_Code').find_element_by_class_name('http_method').find_element_by_tag_name('a').click()
	time.sleep(0.5)
	browser.execute_script("window.scrollTo(document.body.scrollWidth, document.body.scrollHeight)")
	time.sleep(0.5)
